...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        x, cache_x = word_embedding_forward(captions_in, W_embed) # (2)
        h, cache_h = recurrent_forward(x, h0, Wx, Wh, b)    # (3)
        out, cache_out = temporal_affine_forward(h, W_vocab, b_vocab) # (4)

        loss, dout = temporal_softmax_loss(out, captions_out, mask) # (5)

        ####### backward #######
        dout, dW_vocab, db_vocab = temporal_affine_backward(dout, cache_out) # (4)
        dout, dh0, dWx, dWh, db = recurrent_backward(dout, cache_h) # (3)
        dW_embed = word_embedding_backward(dout, cache_x)   # (2)
        _, dW_proj, db_proj = affine_backward(dh0, cache_h0)    # (1)
